...
SCENARIOS_JSON_PATH = BASE_DIR / "scenarios.json"
...
def api_health():
    ...
    return {
        ...
        "cosyvoice_url_configured": bool((os.getenv("COSYVOICE_URL") or "").strip()),
        "cosyvoice_shandong_speaker": bool((os.getenv("COSYVOICE_SPK_ID_SHANDONG") or "").strip()),
        "cosyvoice_shanghai_speaker": bool((os.getenv("COSYVOICE_SPK_ID_SHANGHAI") or "").strip()),
    }
...
async def api_tts(body: TTSRequest):
    ...
    dialect = (body.dialect or "mandarin").strip().lower()
    wav = await cosyvoice_sft_to_wav(text, dialect=dialect)
    if wav:
...
def _load_scenarios_from_json_file():
    ...
    if SCENARIOS_JSON_PATH.is_file():
        with SCENARIOS_JSON_PATH.open("r", encoding="utf-8") as f:
            return json.load(f)
    return {"categories": []}

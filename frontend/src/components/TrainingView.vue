...
let trainingAudioUrl = ''
let trainingSpeechUtterance = null
let trainingTtsController = null
let trainingReadSession = 0
let trainingMounted = true
...
function stopTrainingAudio() {
  ...
  if (trainingTtsController) {
    trainingTtsController.abort()
    trainingTtsController = null
  }
  window.speechSynthesis.cancel()
  if (trainingSpeechUtterance) {
    trainingSpeechUtterance.onend = null
    trainingSpeechUtterance.onerror = null
    trainingSpeechUtterance = null
  }
  if (trainingAudio) {
    trainingAudio.onended = null
    trainingAudio.onerror = null
    trainingAudio.pause()
    trainingAudio.src = ''
    trainingAudio = null
  }
  if (trainingAudioUrl) {
    URL.revokeObjectURL(trainingAudioUrl)
    trainingAudioUrl = ''
  }
}

function isActiveReadSession(session) {
  return trainingMounted && session === trainingReadSession && !!currentScenario.value && !!currentQuestion.value
}

function fallbackBrowserReadQuestion(fullText, session, onDone) {
  if (!isActiveReadSession(session)) return
  window.speechSynthesis.cancel()
  const utterance = new SpeechSynthesisUtterance(fullText)
  trainingSpeechUtterance = utterance
  ...
  utterance.onend = () => {
    if (trainingSpeechUtterance === utterance) trainingSpeechUtterance = null
    if (isActiveReadSession(session)) onDone()
  }
  utterance.onerror = () => {
    if (trainingSpeechUtterance === utterance) trainingSpeechUtterance = null
    if (isActiveReadSession(session)) onDone()
...
async function readCurrentQuestion() {
  stopTrainingAudio()
  const session = trainingReadSession
  ...
  const finish = () => {
    if (isActiveReadSession(session)) isReading.value = false
  }

  try {
    const controller = new AbortController()
    trainingTtsController = controller
    ...
    trainingAudioUrl = url
    const audio = new Audio(url)
    trainingAudio = audio
    audio.onended = () => {
      if (trainingAudioUrl === url) {
        URL.revokeObjectURL(url)
        trainingAudioUrl = ''
      }
      if (trainingAudio === audio) trainingAudio = null
      finish()
    }
    audio.onerror = () => {
      if (trainingAudioUrl === url) {
      ...
      if (trainingAudio === audio) trainingAudio = null
      fallbackBrowserReadQuestion(fullText, session, finish)
    }
    const p = audio.play()
    if (p !== undefined) {
      p.catch(() => {
    ...
    if (isActiveReadSession(session)) trainingTtsController = null
  }
}
